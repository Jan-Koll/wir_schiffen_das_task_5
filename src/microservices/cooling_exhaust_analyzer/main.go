...
func sendNotification(updated_job job) {
	...
	newNotification := notification{Message: updated_job, Sender: "cooling_exhaust_analyzer"}
...
func main() {
	...
	port, exists := os.LookupEnv("COOLING_EXHAUST_ANALYZER_PORT")
	if !exists {
		port = "8082"
...
func postAnalysisHandler(w http.ResponseWriter, r *http.Request) {
	...
	var ok bool
	input.CoolingSystem, ok = responseData["cooling_system"].(string)
	if !ok {
		fmt.Println("Error: cooling_system is not a string", responseData["cooling_system"])
		w.WriteHeader(http.StatusInternalServerError)
		w.Write([]byte("Could not read the response for cooling_system from configuration_manager\n"))
		return
	}
	input.ExhaustSystem, ok = responseData["exhaust_system"].(bool)
	if !ok {
		fmt.Println("Error: exhaust_system is not a bool")
		w.WriteHeader(http.StatusInternalServerError)
		w.Write([]byte("Could not read the response for exhaust_system from configuration_manager\n"))
		return
	}
	input.Status = Queued
