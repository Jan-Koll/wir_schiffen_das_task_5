...
func getAnalysisById(id int) (result job, err error) {
	...
	err = row.Scan(&result.JobId, &result.ConfigurationId, &result.Success, &result.MountingSystem, pq.Array(&result.GearboxOptions), &result.CreatedAt, &result.LastModified, &result.Status)
	return
}
...
func createJob(input job) (result job, err error) {
	result = input
	row := db.QueryRow(
		`INSERT INTO mounting_analyzer (configuration_id, status, mounting_system, gearbox_options)
		 VALUES ($1, $2, $3, $4)
...
func getAnalysesByConfigurationId(configuration_id int) (result []job, err error) {
	...
	for rows.Next() {
		var job job
		err = rows.Scan(&job.JobId, &job.ConfigurationId, &job.Success, &job.MountingSystem, pq.Array(&job.GearboxOptions), &job.CreatedAt, &job.LastModified, &job.Status)
		if err != nil {
