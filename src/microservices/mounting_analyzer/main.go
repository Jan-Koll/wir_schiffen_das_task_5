...
func sendNotification(updated_job job) {
	...
	newNotification := notification{Message: updated_job, Sender: "mounting_analyzer"}
...
func main() {
	...
	port, exists := os.LookupEnv("MOUNTING_ANALYZER_PORT")
	if !exists {
		port = "8084"
...
func postAnalysisHandler(w http.ResponseWriter, r *http.Request) {
	...
	var ok bool
	input.MountingSystem, ok = responseData["mounting_system"].(bool)
	if !ok {
		fmt.Println("Error: mounting_system is not a bool")
		w.WriteHeader(http.StatusInternalServerError)
		w.Write([]byte("Could not read the response for mounting_system from configuration_manager\n"))
		return
	}

	// Type conversion for []interface -> []string
	gearboxInterface, ok := responseData["gearbox_option"].([]interface{})
	if !ok {
		log.Println("Error: gearbox_option is not a slice", responseData["gearbox_option"])
		w.WriteHeader(http.StatusInternalServerError)
		w.Write([]byte("Could not read the response for gearbox_option from configuration_manager\n"))
		return
	}
	for _, value := range gearboxInterface {
		gearboxString, ok := value.(string)
		if !ok {
			log.Println("Warning: Element in gearbox_option is not a string")
			continue
		}
		input.GearboxOptions = append(input.GearboxOptions, gearboxString)
	}
