...
func sendNotification(updated_job job) {
	...
	newNotification := notification{Message: updated_job, Sender: "supply_analyzer"}
...
func main() {
	...
	port, exists := os.LookupEnv("SUPPLY_ANALYZER_PORT")
	if !exists {
		port = "8086"
...
func postAnalysisHandler(w http.ResponseWriter, r *http.Request) {
	...
	OilSystemInterface, ok := responseData["oil_system"].([]interface{})
	if !ok {
		log.Println("Error: oil_system is not a slice", responseData["oil_system"])
		w.WriteHeader(http.StatusInternalServerError)
		w.Write([]byte("Could not read the response for oil_system from configuration_manager\n"))
		return
	}
	for _, value := range OilSystemInterface {
		oilSystemString, ok := value.(string)
		if !ok {
			log.Println("Warning: Element in oil_system is not a string")
			continue
		}
		input.OilSystem = append(input.OilSystem, oilSystemString)
	}

	// Type conversion for []interface -> []string
	FuelSystemInterface, ok := responseData["fuel_system"].([]interface{})
	if !ok {
		log.Println("Error: fuel_system is not a slice", responseData["fuel_system"])
		w.WriteHeader(http.StatusInternalServerError)
		w.Write([]byte("Could not read the response for fuel_system from configuration_manager\n"))
		return
	}
	for _, value := range FuelSystemInterface {
		fuelSystemString, ok := value.(string)
		if !ok {
			log.Println("Warning: Element in fuel_system is not a string")
			continue
		}
		input.FuelSystem = append(input.FuelSystem, fuelSystemString)
	}
	input.Status = Queued
